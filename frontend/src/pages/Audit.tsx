...
import type { AuditReturnContext } from '@/types/navigation'
...
export default function AuditPage() {
  ...
  const handleFireDetailNav = (fireEventId: string) => {
    const ctx = buildAuditReturnContext(currentPage)
    if (!ctx) {
      navigate(`/fires/${encodeURIComponent(fireEventId)}`)
      return
    }
                      ...
                      {AREA_PRESETS.map((opt) => {
                        ...
                        return (
                          <Button
                            ...
                            className={isSelected ? 'bg-secondary/60' : undefined}
                          >
                            {opt.label}
                          </Button>
                        )
