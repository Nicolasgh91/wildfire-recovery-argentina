# Removal log: package.json
Date: 2026-02-12

Removed:
`json
{
  "devDependencies": {
    "autoprefixer": "^10.4.23",
    "postcss": "^8.5.6",
    "tailwindcss": "^4.1.18"
  },
  "name": "wildfire-recovery-argentina",
  "version": "1.0.0",
  "description": "**Plataforma de inteligencia geoespacial para la fiscalización legal y monitoreo de recuperación de zonas afectadas por incendios forestales en Argentina.**",
  "main": "index.js",
  "directories": {
    "doc": "docs",
    "test": "tests"
  },
  "dependencies": {
    "@supabase/supabase-js": "^2.94.1",
    "baseline-browser-mapping": "^2.9.19",
    "browserslist": "^4.28.1",
    "caniuse-lite": "^1.0.30001766",
    "dotenv": "^17.2.3",
    "electron-to-chromium": "^1.5.283",
    "escalade": "^3.2.0",
    "fraction.js": "^5.3.4",
    "h3-js": "^4.4.0",
    "nanoid": "^3.3.11",
    "node-releases": "^2.0.27",
    "picocolors": "^1.1.1",
    "postcss-value-parser": "^4.2.0",
    "source-map-js": "^1.2.1",
    "update-browserslist-db": "^1.2.3"
  },
  "scripts": {
    "test": "echo \"Error: no test specified\" && exit 1"
  },
  "repository": {
    "type": "git",
    "url": "git+https://github.com/Nicolasgh91/wildfire-recovery-argentina.git"
  },
  "keywords": [],
  "author": "",
  "license": "ISC",
  "type": "commonjs",
  "bugs": {
    "url": "https://github.com/Nicolasgh91/wildfire-recovery-argentina/issues"
  },
  "homepage": "https://github.com/Nicolasgh91/wildfire-recovery-argentina#readme"
}

`

Reason:
Root-level Node manifest had no references in repo; frontend has its own package.json and scripts.

Recovery:
Restore package.json with the content above and re-run 
pm install at repo root if needed.
