"""
Recovery Task: Monitoreo de recuperación de vegetación post-incendio - FINAL CORRECTED VERSION
Schema v5 compatible with proper table relationships
"""
...
def analyze_episode_recovery(self, episode_id, months_after=None):
    ...
    FINAL CORRECTED: Usa fire_episode_events table para la relación.
    """
    from app.db.session import SessionLocal
    from sqlalchemy import text
    ...
    db = SessionLocal()
    try:
        # Seleccionar evento representativo usando fire_episode_events - FINAL CORRECTED
...
def batch_episode_recovery_analysis(self, max_episodes=50, recent_only=False, carousel_only=False):
    """
    Enhanced batch processing with comprehensive monitoring - FINAL CORRECTED VERSION.
    Schema v5 compatible with proper fire_episode_events relationships.
    """
    logger = logging.getLogger(__name__)
    ...
    db = SessionLocal()
    try:
        # Build query based on parameters - FINAL CORRECTED
        ...
        # Get episodes with representative events - FINAL CORRECTED
