"""
Recovery Task: Monitoreo de recuperación de vegetación post-incendio - SCHEMA V5 CORRECTED
"""
...
def analyze_episode_recovery(self, episode_id, months_after=None):
    ...
    SCHEMA V5 CORRECTED: Usa max_frp y geometrías PostGIS reales.
    """
    from app.db.session import SessionLocal
    from sqlalchemy import text
    ...
    db = SessionLocal()
    try:
        # Seleccionar evento representativo (máximo max_frp) - SCHEMA V5 CORRECTED
...
def batch_episode_recovery_analysis(self, max_episodes=50, recent_only=False, carousel_only=False):
    """
    Enhanced batch processing with comprehensive monitoring - SCHEMA V5 CORRECTED.
    Usa geometrías PostGIS reales de eventos representativos y max_frp correcto.
    """
    logger = logging.getLogger(__name__)
    ...
    db = SessionLocal()
    try:
        # Build query based on parameters - SCHEMA V5 CORRECTED
        ...
        # Get episodes with representative events - SCHEMA V5 CORRECTED
