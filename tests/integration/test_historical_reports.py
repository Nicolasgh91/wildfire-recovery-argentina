...
def valid_bbox():
    ...
    return BoundingBox(west=-60.5, south=-27.0, east=-60.3, north=-26.8)
...
def valid_request(valid_bbox):
    ...
    return HistoricalReportRequest(
        ...
        report_config=ReportConfig(
            post_fire_frequency=PostFireFrequency.ANNUAL, max_images=5
        ),
...
class TestBoundingBox:
    ...
    def test_valid_bbox(self):
        ...
        bbox = BoundingBox(west=-60.5, south=-27.0, east=-60.3, north=-26.8)
    ...
    def test_invalid_west_east(self):
        ...
        with pytest.raises(ValueError, match="west debe ser menor que east"):
            BoundingBox(
                west=-60.0, south=-27.0, east=-60.5, north=-26.8  # Mayor que east
            )
    ...
    def test_invalid_south_north(self):
        ...
        with pytest.raises(ValueError, match="south debe ser menor que north"):
            BoundingBox(
                west=-60.5, south=-26.0, east=-60.3, north=-27.0  # Mayor que north
            )
    ...
    def test_out_of_range_longitude(self):
        ...
        with pytest.raises(ValueError):
            BoundingBox(
                west=-200.0, south=-27.0, east=-60.3, north=-26.8  # Fuera de rango
            )
    ...
    def test_out_of_range_latitude(self):
        ...
        with pytest.raises(ValueError):
            BoundingBox(
                west=-60.5, south=-100.0, east=-60.3, north=-26.8  # Fuera de rango
            )
...
class TestHistoricalReportRequest:
    ...
    def test_future_fire_date_rejected(self, valid_bbox):
        ...
        with pytest.raises(ValueError, match="no puede ser una fecha futura"):
            HistoricalReportRequest(fire_date=future_date, bbox=valid_bbox)

    def test_default_area_name(self, valid_bbox):
        """Si no se especifica nombre, debe usar default."""
        request = HistoricalReportRequest(fire_date=date(2020, 8, 15), bbox=valid_bbox)
        ...
        with pytest.raises(ValueError, match="start debe ser anterior"):
            HistoricalReportRequest(
                ...
                date_range=DateRange(
                    start=date(2022, 1, 1), end=date(2021, 1, 1)  # Antes de start
                ),
            )
...
class TestHistoricalReportResponse:
    ...
    def test_minimal_response(self):
        ...
        response = HistoricalReportResponse(
            report_id="RPT-HIST-20250129-ABC123", status=ReportStatus.PROCESSING
        )
...
class TestGEEServiceMock:
    ...
    def test_health_check(self):
        ...
        with patch.object(GEEService, "authenticate", return_value=True):
            with patch.object(
                GEEService, "_rate_limited_request", return_value={"id": "test"}
            ):
                service = GEEService()
                # Mock the ee module
...
class TestVAEServiceMock:
    ...
    def test_recovery_classification(self):
        ...
        try:
            from app.services.vae_service import RecoveryStatus, VAEService
        except ImportError:
...
class TestERSServiceMock:
    ...
    def test_report_id_generation(self):
        ...
        service = ERSService(
            gee_service=mock_gee, vae_service=mock_vae, storage_service=mock_storage
        )
...
class TestUtilities:
    ...
    def test_months_between(self):
        ...
        # 18 meses
        assert (
            service._months_between(date(2020, 6, 15), date(2022, 1, 15)) == 19
        )  # Aproximado
