...
try:
    from .gee_service import GEEService, NDVIResult, GEEImageNotFoundError
...
class VAEService:
    ...
    def analyze_recovery(
        ...
        logger.info(f"Analyzing recovery for {fire_event_id}, {months_after} months after fire")
        ...
        if baseline_ndvi > 0:
            ...
            recovery_pct = 100 if current_ndvi > NDVI_THRESHOLDS["moderate_vegetation"] else 0
    ...
    def detect_land_use_change(
        ...
        action = self._get_recommended_action(change_type, severity, requires_verification)
    ...
    def _get_baseline_ndvi(self, bbox: Dict[str, float], fire_date: date) -> float:
        ...
        try:
            ...
            image = self._gee.get_best_image(collection, target_date=fire_date - timedelta(days=15))
    ...
    def _get_expected_recovery(self, months_after: int) -> float:
        ...
        if months_after <= sorted_months[0]:
            return EXPECTED_RECOVERY[sorted_months[0]] * (months_after / sorted_months[0])
    ...
    def _estimate_geometric_index(self, current_ndvi: float, baseline_ndvi: float) -> float:
        """
