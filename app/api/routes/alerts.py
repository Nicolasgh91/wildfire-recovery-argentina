...
from app.schemas.alerts import ParkCapacityAlertRequest, ParkCapacityAlertResponse
...
def create_park_capacity_alert(
    request: ParkCapacityAlertRequest,
    db: Session = Depends(get_db)
) -> ParkCapacityAlertResponse:
    protected_area = db.query(ProtectedArea).filter(
        ProtectedArea.id == request.protected_area_id
    ).first()

    if not protected_area:
        raise HTTPException(status_code=404, detail="Protected area not found")

    carrying_capacity = request.carrying_capacity or protected_area.carrying_capacity
    if not carrying_capacity:
        raise HTTPException(status_code=400, detail="Carrying capacity not defined for protected area")
    ...
    alert = ParkAlert(
        ...
        climate_summary=json.dumps(request.climate.model_dump()) if request.climate else None,
