...
from app.models.visitor import Shelter, VisitorLog, VisitorLogCompanion, VisitorLogRevision
...
def create_visitor_log(
    ...
    for companion in request.companions:
        log.companions.append(VisitorLogCompanion(
...
def update_visitor_log(
    ...
    if not can_edit_log(log.first_submitted_at or log.created_at, now):
        raise HTTPException(status_code=403, detail="Edit window expired (30 minutes)")
    ...
    before_snapshot = {
        ...
        "contact_phone": log.contact_phone,
        "companions": [
            {"full_name": c.full_name, "age_range": c.age_range, "document": c.document}
            for c in log.companions
        ],
    }
...
@router.get(
    "/visitor-logs/export",
    summary="Export visitor logs (UC-12)"
)
def export_visitor_logs(
    ...
    output.seek(0)
    filename = f"visitor_logs_{datetime.utcnow().strftime('%Y%m%d_%H%M%S')}.csv"
