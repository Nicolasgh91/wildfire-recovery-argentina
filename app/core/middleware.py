...
class DeprecationMiddleware(BaseHTTPMiddleware):
    ...
    """
    def __init__(self, app, date: str = "2026-04-30", link: str = "https://docs.forestguard.ar/migration-guide"):
        super().__init__(app)
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        # RFC 8594 Headers
        response.headers["Deprecation"] = f"date=\"{self.deprecation_date}\"" # Or version
...
class LatencyMonitorMiddleware(BaseHTTPMiddleware):
    ...
    def _check_slo(self, path: str, duration: float):
        ...
        for slo_path, limit in SLO_THRESHOLDS.items():
            ...
            if path == slo_path or (slo_path.endswith("/") and path.startswith(slo_path)):
        ...
        if budget and duration > budget:
            logger.warning(
                f"🚨 SLO BREACHED | Endpoint: {path} | Duration: {duration:.4f}s | Budget: {budget}s"
            )
