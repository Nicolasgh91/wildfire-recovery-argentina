...
class CitizenReport(Base, UUIDMixin, TimestampMixin):
    ...
    # Reported location
    reported_location = Column(Geography(geometry_type='POINT', srid=4326), nullable=False, index=True)
...
class LandUseChange(Base, UUIDMixin, TimestampMixin):
    ...
    # Foreign keys
    fire_event_id = Column(UUID(as_uuid=True), ForeignKey('fire_events.id'), nullable=False, index=True)
    monitoring_record_id = Column(UUID(as_uuid=True), ForeignKey('vegetation_monitoring.id'))
    ...
    monitoring_record = relationship("VegetationMonitoring", back_populates="land_use_change")
