...
class LandUseAudit(Base, UUIDMixin, TimestampMixin):
    ...
    queried_location = Column(Geography(geometry_type='POINT', srid=4326), nullable=False, index=True)
    ...
    violation_severity = Column(String(20))  # 'none', 'low', 'medium', 'high', 'critical'
...
class LandCertificate(Base, UUIDMixin, TimestampMixin):
    ...
    # Reference to audit
    audit_id = Column(UUID(as_uuid=True), ForeignKey('land_use_audits.id'), nullable=False)
