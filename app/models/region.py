...
class Region(Base):
  ...
  # Geometría: MultiPolygon para soportar islas o territorios discontinuos
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            f"category IN {REGION_CATEGORIES}",
            name="ck_regions_valid_category"
        ),
    )
...
class ProtectedArea(Base, UUIDMixin, TimestampMixin):
    ...
    official_name = Column(
        ...
        nullable=False, 
        index=True,
        doc="Nombre oficial del área protegida"
    )
    
    alternative_names = Column(
        ARRAY(Text),
        doc="Nombres alternativos o históricos"
    )
    ...
    centroid = Column(
        ...
        doc="Centro geográfico del área"
    )
    
    area_hectares = Column(
        Float,
        doc="Superficie total en hectáreas"
    )
    ...
    jurisdiction = Column(
        String(50),
        doc="Jurisdicción: national, provincial, municipal, private"
    )
    
    province = Column(
        String(100), 
        index=True,
        doc="Provincia donde se ubica el área"
    )
    
    department = Column(
        String(100),
        doc="Departamento/Partido donde se ubica"
    )
    ...
    established_date = Column(
        Date,
        doc="Fecha de creación/establecimiento del área"
    )
    
    managing_authority = Column(
        String(200),
        doc="Autoridad de gestión (APN, Gobierno Provincial, etc.)"
    )
    ...
    wdpa_id = Column(
        Integer, 
        unique=True,
        doc="ID en World Database on Protected Areas"
    )
    
    iucn_category = Column(
        String(10),
        doc="Categoría IUCN (Ia, Ib, II, III, IV, V, VI)"
    )
    
    source_dataset = Column(
        String(100),
        doc="Dataset de origen (IGN, APN, WDPA)"
    )
    
    source_url = Column(
        Text,
        doc="URL del dataset original"
    )
    ...
    __table_args__ = (
        ...
        Index('idx_protected_areas_simplified', simplified_boundary, postgresql_using='gist'),
        # Validar categorías
        CheckConstraint(
        ...
        CheckConstraint(
            "prohibition_years IN (30, 60)",
            name="ck_protected_areas_valid_prohibition"
        ),
    )
    ...
    def __repr__(self) -> str:
        return f"<ProtectedArea(name='{self.official_name}', category='{self.category}')>"
...
class FireProtectedAreaIntersection(Base, UUIDMixin, TimestampMixin):
    ...
    intersection_area_hectares = Column(
        Float,
        doc="Superficie de la intersección en hectáreas"
    )
    
    overlap_percentage = Column(
        Float,
        doc="Porcentaje del área del fuego dentro del área protegida"
    )
    ...
    fire_date = Column(
        Date, 
        nullable=False,
        doc="Fecha del incendio (inicio de la prohibición)"
    )
    ...
    fire_event: Mapped["FireEvent"] = relationship(
        "FireEvent", 
        back_populates="protected_area_intersections"
    )
    
    protected_area: Mapped["ProtectedArea"] = relationship(
        "ProtectedArea", 
        back_populates="fire_intersections"
    )
    ...
    __table_args__ = (
        # Evitar duplicados
        UniqueConstraint(
            'fire_event_id', 'protected_area_id',
            name='uq_fire_protected_area'
        ),
