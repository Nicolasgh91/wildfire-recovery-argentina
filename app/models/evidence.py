from sqlalchemy import Boolean, Column, DateTime, Date, Float, ForeignKey, Integer, SmallInteger, String, Text, Time
...
class SatelliteImage(Base, UUIDMixin, TimestampMixin):
    """
    Imágenes Sentinel-2 (pre/post fuego) almacenadas en Cloudflare R2.
    """
    
    __tablename__ = 'satellite_images'
    
    # Foreign key
    fire_event_id = Column(UUID(as_uuid=True), ForeignKey('fire_events.id', ondelete='CASCADE'), nullable=False)
    ...
    quality_score = Column(String(20))  # 'excellent', 'good', 'fair', 'poor', 'unusable'
    ...
    vegetation_monitoring = relationship("VegetationMonitoring", back_populates="satellite_image")
...
class VegetationMonitoring(Base, UUIDMixin, TimestampMixin):
    ...
    satellite_image = relationship("SatelliteImage", back_populates="vegetation_monitoring")
